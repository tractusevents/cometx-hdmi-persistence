#!/bin/sh
# Bounded direct video-selector trial; adapted from the recorded 2026-09-08 experiment.
# MCU is paused throughout D -> C -> B. This is not a normal mailbox switch.
set -eu
...
check_state() {
    actual=$(read8 0x00 0x01)
    printf '0001=%s expected=%s\n' "$actual" "$expected_mux"
    [ "$actual" = "$expected_mux" ] || return 1
    [ "$(read8 0x00 0x00)" = "$mode" ] || return 1
    [ "$(read8 0xff 0xff)" = 0x00 ] || return 1
    [ "$(cat "$node/channel")" = 'Current Channel : 1' ] || return 1
    while read -r hi lo value; do
        [ "$hi $lo" != '0x00 0x01' ] || continue
        actual=$(read8 "$hi" "$lo")
        expected=$value
        ...
        [ "$actual" = "$expected" ] && [ "$((actual & 2))" = 2 ] || return 1
    done < "$guard/saved"
    status_b=$(read8 0x03 0x14)
    printf '0314=%s\n' "$status_b"
    if [ "$lab" != 1 ]; then [ "$((status_b & 0x22))" = 34 ] || return 1; fi
}
